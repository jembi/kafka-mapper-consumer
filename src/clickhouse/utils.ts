...
const insertRAWQuery = (tableName: string, data: object) => {
  const filteredData = Object.keys(data).reduce((obj, key) => {
    if (data[key] !== null) {
      obj[key] = data[key]
    }
...
export const loadRawDataIntoClickhouse = (table: Table) =>
  clickhouse.insert(insertRAWQuery(table.name, table.rows)).toPromise()

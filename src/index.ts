...
const shouldRegisterMediator =
  (process.env.REGISTER_MEDIATOR ?? 'false') === 'true'
...
const run = async (newFhirMappings?) => {
  ...
  if (shouldRegisterMediator && !newFhirMappings) {
    console.log(
      'Registering as an OpenHIM Mediator. The OpenHIM will be used to supply latest config not the config file, the config file will provide the default config to the OpenHIM.'
    )
  ...
  const topics = fhirMappings.map(fhirMapping =>
    fhirMapping.resourceType.toLowerCase()
  )
...
run().catch((e: Error) =>
  console.error(`[kafka-mapper-consumer] ${e.message}`, e)
)
